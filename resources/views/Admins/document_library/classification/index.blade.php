   ...
   <div class="container-fluid">
      <div class="row mb-2">
         <div class="col-sm-6 mt-4">
            <h1 class="m-0 text-dark"style="text-decoration: bold;">Document Library Classification Management</h1>
         </div>
...
<section class="content">
   ...
   <!-- <form method="get" action="">
	  ...
	  <div class="float-sm-right  mb-3">
        <a href="{{ route('document_library.classification.create')}}" class="btn btn-primary " style="margin-bottom:5px;">Create Document Classification</a>
      </div>
         ...
         @foreach($data as $key=>$document)
          ...
          <td>{{($data->perPage() * ($data->currentPage() - 1))+ ($key+1)}}</td>
              ...
              <td style="min-width:118px;">
                  <a href="{{route('document_library.classification.edit',$document->id)}}" class="btn btn-info"> 
    ...
    swal({
         ...
         closeOnConfirm: false,
            ...
            $.ajax({
                  ...
                  url: "/admin/document_library_classification/delete/" +id,
                  success: function(data) {
                        swal({
                          ...
                          //timer: 3000
                              ...
                              window.location.replace("{{url('/admin/document_library_classification')}}");
                          });
                  }
            });
         });
...
<script>
  function statefunction(id){
        swal({
          ...
          jQuery.ajax({
            type: 'get',
            url:  "{{url('/admindocument_library_classification/status')}}/"+id,
            success: function(data) {
                if(data == 1){
