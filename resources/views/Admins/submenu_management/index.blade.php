	...
	<section class="content">
      ...
      <div class="d-block">
        ...
        <div class="col-lg-3" style="margin-right:4px">
			 <select name="main_id" id="main_id" class="form-control" onchange="getSubmenu(this.value,'','{{ route("page.getSubmenu") }}');">
					  
					<option value="">Select Menu</option>
					@foreach($menus as $key=>$menu) 
					<option @if($key ==Request::get('main_id')) selected="selected" @endif value="{{$key}}">{{$menu}}</option>
					@endforeach
					</select>
          </div>
		  <div class="col-lg-3" style="margin-right:4px">
			 <select name="parent_id" id="submenu_id" class="form-control">
		...
		<div class="float-sm-right mt-4 mb-3">
			<a href="{{ route('submenu.create')}}" class="btn btn-primary " style="margin-bottom:5px;">Create CMS</a>
		  </div>
		  ...
		  <table class="table table-bordered table-striped">
			 ...
			 <tr>
				...
				<td>{{strip_tags(\Illuminate\Support\Str::limit($page->description, 75, $end='...'))}}</td>
				<td>{{strip_tags(\Illuminate\Support\Str::limit($page->description_ar, 75, $end='...'))}}</td>
				<td style="min-width:155px;">
					<a href="{{route('sub_menu.history',$page->id)}}" class="btn btn-warning" title="History"><i class="text-white fas fa-history" aria-hidden="true" ></i></a>
				   <a href="{{route('submenu.edit',$page->id)}}" class="btn btn-info" title="Edit"> 
           ...
           @if($page->status == '1')
					...
					@endif
				   <a onclick="deleteConfirmation(`{{route('submenu.delete',$page->id)}}`)" url="{{route('submenu.edit',$page->id)}}" class="btn btn-danger" title="Delete"> 
...
<script>
  function statefunction(id){
        swal({
          ...
          jQuery.ajax({
            type: 'get',
            url:  "{{url('/admin/submenu/status')}}/"+id,
            success: function(data) {
                if(data == 1){
        ...
        {
           window.location.replace("{{route('submenu.index')}}");
        }
        });
      }
