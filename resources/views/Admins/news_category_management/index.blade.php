...
<div class="row">
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6 mt-4">
            <h1 class="m-0 text-dark">News Category Management</h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
   ...
   <div class="container-fluid">
	   ...
	   <div class="float-sm-right mt-4 mb-3">
        <a href="{{ route('news_category.create')}}" class="btn btn-primary " style="margin-bottom:5px;">Create News Category</a>
      </div>
        ...
        <tr>
		  ...
		  <th>Action</th>
            ...
            @foreach($news_category as $key =>$category)
				...
				<tr>
        <td style="width:50px;">{{($news_category->perPage() * ($news_category->currentPage() - 1))+ ($key+1)}}</td>
				  <td>{{$category->name}}</td>
				  <td>{{$category->name_ar}}</td>
				       
				  <td>
				  <a href="{{ route('news_category.edit',$category->id)}}" class="btn btn-info" title="Edit"><i class="fa fa-edit" aria-hidden="true"></i></a>
 ...
 function deleteConfirmation(id) {
    swal({
        ...
        if(isConfirm){
         $.ajax({
                ...
                url: "/admin/news_category/delete/" +id,
                ...
                swal({
                     ...
                     window.location.replace("{{route('news_category.index')}}");
                     });
                }
                });}
         });
  ...
  function statefunction(id){
        swal({
          ...
          jQuery.ajax({
            type: 'get',
            url:  "{{url('/admin/news_category/status')}}/"+id,
            success: function(data) {
                if(data == 1){
